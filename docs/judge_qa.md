...
### 2. Why Gemini 3.5 Flash instead of OpenAI GPT-4o or Claude 3.5 Sonnet?
...
1. **Audio ASR Failure:** If audio recording or Whisper fails, the Caller UI immediately expands a text fallback manual description box.
...
2. **Server-Side API Keys:** All communication with external APIs (OpenAI, Gemini) is proxied through our secure FastAPI server. No API keys are exposed to the client-side browser, preventing credentials theft."
...
1. **Edge Deployment:** Compiling Whisper.cpp and a quantized LLM (like Llama-3-8B-Instruct via Llama.cpp) to run directly on the dispatcher's local machine or edge server. This would make the entire system functional offline.
